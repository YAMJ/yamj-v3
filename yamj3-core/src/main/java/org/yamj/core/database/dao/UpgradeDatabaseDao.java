...
public class UpgradeDatabaseDao extends HibernateDao {
    ...
    // PATCHES
	 @SuppressWarnings("unchecked")
      public void patchDatabaseCompatibilityHSQL() {
        currentSession()
        .createSQLQuery("SET DATABASE SQL SYNTAX MYS TRUE")
        .executeUpdate();
	  }
	 ...
	 @SuppressWarnings("unchecked")
      public void patchDatabaseUpdateLibraries() {
        currentSession()
        .createSQLQuery("insert into series_libraries (series_id, library_id) select distinct ser.id, sd.library_id "
		...
		+ "join series ser on sea.series_id = ser.id "
		+ "ON DUPLICATE KEY UPDATE series_libraries.series_id = series_libraries.series_id")
        .executeUpdate();
		...
		+ "ON DUPLICATE KEY UPDATE videodata_libraries.data_id = videodata_libraries.data_id")
		.executeUpdate();
