...
public class MetadataDao extends HibernateDao {
    ...
    public void handleModifiedSources(VideoData videoData) {
        if (!videoData.hasModifiedSource()) return; 
    ...
    public void handleModifiedSources(Season season) {
        if (!season.hasModifiedSource()) return; 
        markLocatedArtworkAsDeleted(season.getArtworks(), season.getModifiedSources());
        
        for (VideoData videoData : season.getVideoDatas()) {
            boolean changed = false;
            for (String sourceDb : videoData.getModifiedSources()) {
                changed = (changed || season.removeSourceDbId(sourceDb));
            }
            ...
            if (changed) {
                ...
                updateEntity(videoData);
            }
        }
    }

    public void handleModifiedSources(Series series) {
        if (!series.hasModifiedSource()) return; 
    ...
    public void handleModifiedSources(Person person) {
        if (!person.hasModifiedSource()) return; 
        person.getFilmography().clear();
        markLocatedArtworkAsDeleted(person.getPhoto(), person.getModifiedSources());
    }

    private void markLocatedArtworkAsDeleted(List<Artwork> artworks, Set<String> modifiedSources) {
        for (Artwork artwork : artworks) {
            markLocatedArtworkAsDeleted(artwork, modifiedSources);
       }
    }

    private void markLocatedArtworkAsDeleted(Artwork artwork, Set<String> modifiedSources) {
        artwork.setStatus(StatusType.UPDATED);
        updateEntity(artwork);
        
        for (ArtworkLocated located : artwork.getArtworkLocated()) {
            if (located.getUrl() != null && modifiedSources.contains(located.getSource())) {
                located.setStatus(StatusType.DELETED);
                updateEntity(located);
            }
        }
    }
}
