...
public class StageDirectory  extends AbstractAuditable implements Serializable {
    ...
    public boolean equals(Object other) {
        ...
        if ( !(other instanceof StageDirectory) ) return false;
        StageDirectory castOther = (StageDirectory)other;

        if (!StringUtils.equals(this.directoryPath, castOther.directoryPath)) return false;

        if (this.library == null && castOther.library == null) return true;
        if (this.library == null) return false;  
        if (castOther.library == null) return false;
