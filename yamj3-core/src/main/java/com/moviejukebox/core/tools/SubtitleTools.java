...
public final class SubtitleTools implements InitializingBean {
    ...
    public static void setMovieSubtitles(VideoData movie, Collection<String> subtitles) {
     ...
     * @param subtitles
     * @param newSubtitle
    public void addSubtitle(Collection subtitles, String newSubtitle) {
        // determine the language
        String language = languageTools.determineLanguage(newSubtitle);

        if (StringUtils.isNotBlank(language) && !isSkippedSubtitle(language)) {
            if (subtitles.isEmpty()) {
                subtitles.add(e)
            }
